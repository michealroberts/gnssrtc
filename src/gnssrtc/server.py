...
class GNSSStratum1NTPServer(NTPServer):
    ...
    def __enter__(self) -> "GNSSStratum1NTPServer":
        # initial draw
        self.console.clear()
        self.console.print(self._render_panels())
        # schedule our single asyncio task
        loop = asyncio.get_event_loop()
        self._task = loop.create_task(self._gps_and_refresh_loop())
        return self
    ...
    def __exit__(
        ...
        # cancel the task
        self._task.cancel()
        # one final draw to leave panels on screen
        self.console.clear()
        self.console.print(self._render_panels())

    async def _gps_and_refresh_loop(self) -> None:
        """
